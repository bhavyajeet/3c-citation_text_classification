...
output_file_name = str(sys.argv[5]) + "_" + str(sys.argv[1]).split('/')[1] + "_" + str(sys.argv[2]) + "_" + str(sys.argv[3]) + "_" + str(sys.argv[4]) + ".txt"
...
class LMClass(torch.nn.Module):
    def __init__(self):
        ...
        self.l1 = LMModel
        self.lstm = torch.nn.LSTM(input_size = 768, hidden_size = 768, bidirectional=True, dropout=drop_out, batch_first=True)
        self.fc = torch.nn.Linear(768 * 2, 6)
    ...
    def forward(self, data):    
        ...
        print ("pooler")
        print (pooler.shape)
        packed_output, (hidden, cell) = self.lstm(pooler)
        
        out_forward =  packed_output[range(len(output)), text_len - 1, :dimension]
